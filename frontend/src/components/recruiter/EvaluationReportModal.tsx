                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-xs border-collapse">
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 font-semibold text-slate-700 dark:text-slate-300">
                        {(combined.stages || [
                          { stage_num: 1, name: 'ATS Resume Screening', score: ats?.ats_score ?? 0, threshold: '80%', status: (ats?.ats_score ?? 0) >= 80 ? 'Passed (≥80%)' : 'Under Review', is_passed: (ats?.ats_score ?? 0) >= 80 },
                          { stage_num: 2, name: 'Online Assessment', score: assessData?.score ?? null, threshold: '70%', status: assessData?.score != null ? (assessData.score >= 70 ? 'Passed (≥70%)' : 'Failed (<70%)') : 'Not Conducted / Waived', is_passed: assessData?.score != null ? assessData.score >= 70 : true },
