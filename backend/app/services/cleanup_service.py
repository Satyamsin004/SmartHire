...
class CleanupService:
    ...
    async def execute_full_cleanup(db: AsyncSession) -> Dict[str, Any]:
        ...
        deletion_steps = [
            ...
            ("interview_transcript_segments", f"DELETE FROM interview_transcript_segments WHERE session_id IN (SELECT id FROM interview_sessions WHERE candidate_id IN ({c_sql}))"),
            ("interview_visual_observations", f"DELETE FROM interview_visual_observations WHERE session_id IN (SELECT id FROM interview_sessions WHERE candidate_id IN ({c_sql}))"),
            ("interview_speech_metrics", f"DELETE FROM interview_speech_metrics WHERE session_id IN (SELECT id FROM interview_sessions WHERE candidate_id IN ({c_sql}))"),
            ("interview_visual_metrics", f"DELETE FROM interview_visual_metrics WHERE session_id IN (SELECT id FROM interview_sessions WHERE candidate_id IN ({c_sql}))"),
            ("interview_filler_events", f"DELETE FROM interview_filler_events WHERE session_id IN (SELECT id FROM interview_sessions WHERE candidate_id IN ({c_sql}))"),
            ("interview_integrity_events", f"DELETE FROM interview_integrity_events WHERE session_id IN (SELECT id FROM interview_sessions WHERE candidate_id IN ({c_sql}))"),
            ("interview_vision_analysis", f"DELETE FROM interview_vision_analysis WHERE is_test_data = TRUE OR environment = 'TEST' OR session_id IN (SELECT id FROM interview_sessions WHERE candidate_id IN ({c_sql}))"),
            ("interview_transcripts", f"DELETE FROM interview_transcripts WHERE is_test_data = TRUE OR environment = 'TEST' OR session_id IN (SELECT id FROM interview_sessions WHERE candidate_id IN ({c_sql}))"),
            ("interview_recordings", f"DELETE FROM interview_recordings WHERE is_test_data = TRUE OR environment = 'TEST' OR session_id IN (SELECT id FROM interview_sessions WHERE candidate_id IN ({c_sql}))"),
            ...
            ("assessment_questions", f"DELETE FROM assessment_questions WHERE session_id IN (SELECT id FROM assessment_sessions WHERE candidate_id IN ({c_sql}) OR title LIKE '%Test%' OR title LIKE '%Placement Mock%')"),
            ("candidate_question_history", f"DELETE FROM candidate_question_history WHERE candidate_id IN ({c_sql})"),
            ("assessment_sessions", f"DELETE FROM assessment_sessions WHERE candidate_id IN ({c_sql}) OR title LIKE '%Test%' OR title LIKE '%Placement Mock%'"),
